...
#[cfg(all(feature = "mimalloc", not(feature = "dhat-heap")))]
static CURRENT_USAGE: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);

#[cfg(all(feature = "mimalloc", not(feature = "dhat-heap")))]
unsafe fn check_and_perform_gc(usage: u64) {
    let limit = clash::ALLOCATOR_LIMIT.load(std::sync::atomic::Ordering::Relaxed);
    if limit > 0 && usage > limit {
        static GC_IN_PROGRESS: std::sync::atomic::AtomicBool =
...
unsafe impl std::alloc::GlobalAlloc for LimitedAllocator {
    unsafe fn alloc(&self, layout: std::alloc::Layout) -> *mut u8 {
        ...
        if !ptr.is_null() {
            let limit =
                clash::ALLOCATOR_LIMIT.load(std::sync::atomic::Ordering::Relaxed);
            if limit > 0 {
                let old = CURRENT_USAGE.fetch_add(
                    layout.size() as u64,
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: std::alloc::Layout) {
        ...
        let limit =
            clash::ALLOCATOR_LIMIT.load(std::sync::atomic::Ordering::Relaxed);
        if limit > 0 {
            let old = CURRENT_USAGE.fetch_sub(
                layout.size() as u64,
                std::sync::atomic::Ordering::Relaxed,
            );
