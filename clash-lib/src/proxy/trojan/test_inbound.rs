use std::sync::Arc;
use std::time::Duration;
use std::collections::HashMap;
use std::io::Write;

use tokio::net::TcpListener;
use tokio::io::{AsyncReadExt, AsyncWriteExt};
use tempfile::NamedTempFile;

use crate::app::dispatcher::Dispatcher;
use crate::app::outbound::manager::OutboundManager;
use crate::app::router::Router;
use crate::app::dns::SystemResolver;
use crate::proxy::direct;
use crate::config::def::RunMode;
use crate::app::profile::ThreadSafeCacheFile;
use crate::app::dispatcher::StatisticsManager as StatsManager;
use crate::proxy::trojan::inbound::TrojanInbound;
use crate::proxy::inbound::InboundHandlerTrait;
use crate::proxy::trojan::{Handler as TrojanOutbound, HandlerOptions};
use crate::proxy::HandlerCommonOptions;
use crate::proxy::transport::TlsClient;
use crate::proxy::OutboundHandler;

const CERT_PEM: &str = r#"-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"#;

const KEY_PEM: &str = r#"-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
"#;

#[tokio::test]
async fn test_trojan_inbound_outbound() {
    let _ = rustls::crypto::aws_lc_rs::default_provider().install_default();
    // 1. Setup certs
    let mut cert_file = NamedTempFile::new().unwrap();
    cert_file.write_all(CERT_PEM.as_bytes()).unwrap();
    let cert_path = cert_file.path().to_str().unwrap().to_owned();

    let mut key_file = NamedTempFile::new().unwrap();
    key_file.write_all(KEY_PEM.as_bytes()).unwrap();
    let key_path = key_file.path().to_str().unwrap().to_owned();

    // 2. Setup Dispatcher
    let resolver = Arc::new(SystemResolver::new(false).unwrap());
    let cache_file = NamedTempFile::new().unwrap();
    let cache_store = ThreadSafeCacheFile::new(cache_file.path().to_str().unwrap(), false);

    let direct = Arc::new(direct::Handler::new("DIRECT"));
    let outbound_manager = Arc::new(OutboundManager::new(
        vec![direct],
        vec![],
        HashMap::new(),
        vec!["DIRECT".to_string()],
        resolver.clone(),
        cache_store,
        ".".into(),
    ).await.unwrap());

    let router = Arc::new(Router::new(
        vec![],
        HashMap::new(),
        resolver.clone(),
        None, None, None, ".".into()
    ).await);

    let stats_mgr = StatsManager::new();

    let dispatcher = Arc::new(Dispatcher::new(
        outbound_manager,
        router,
        resolver.clone(),
        RunMode::Direct, // Direct mode -> routes to DIRECT proxy
        stats_mgr,
        None,
    ));

    // 3. Start Echo Server
    let echo_listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let echo_addr = echo_listener.local_addr().unwrap();
    
    tokio::spawn(async move {
        loop {
            let (mut socket, _) = echo_listener.accept().await.unwrap();
            tokio::spawn(async move {
                let mut buf = [0u8; 1024];
                loop {
                    let n = match socket.read(&mut buf).await {
                        Ok(n) if n == 0 => return,
                        Ok(n) => n,
                        Err(_) => return,
                    };
                    if socket.write_all(&buf[..n]).await.is_err() {
                        return;
                    }
                }
            });
        }
    });

    // 4. Start Trojan Inbound
    let trojan_listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let trojan_addr = trojan_listener.local_addr().unwrap();
    // Drop listener to free port, there's a race but likely fine for test
    drop(trojan_listener);

    let trojan_inbound = TrojanInbound::new(
        trojan_addr,
        true,
        dispatcher.clone(),
        Arc::new(crate::common::auth::PlainAuthenticator::new(vec![])),
        None,
        "password".to_string(),
        cert_path,
        key_path,
        vec!["h2".to_string(), "http/1.1".to_string()],
        None,
        None,
    );

    tokio::spawn(async move {
        trojan_inbound.listen_tcp().await.unwrap();
    });

    // Wait for listener to start
    tokio::time::sleep(Duration::from_millis(100)).await;

    // 5. Start Trojan Outbound (Client)
    let tls_client = TlsClient::new(
        true, // skip verify
        "localhost".to_string(),
        Some(vec!["h2".to_string(), "http/1.1".to_string()]),
        None,
    );

    let opts = HandlerOptions {
        name: "trojan-out".to_string(),
        common_opts: HandlerCommonOptions::default(),
        server: "127.0.0.1".to_string(),
        port: trojan_addr.port(),
        password: "password".to_string(),
        udp: false,
        tls: Some(Box::new(tls_client)),
        transport: None,
        quic_client: None,
        tquic_client: None,
    };

    let client = TrojanOutbound::new(opts);

    // 6. Connect and Test
    // Use resolver to create a dummy stream to the destination
    // But client.connect_stream needs session and resolver
    // Actually we can use client.proxy_stream if we had one?
    // TrojanOutbound::connect_stream establishes connection to Trojan Server, then handshake.
    
    let sess = crate::session::Session {
        destination: crate::session::SocksAddr::Ip(echo_addr),
        ..Default::default()
    };

    let stream = client.connect_stream(
        &sess,
        resolver.clone(),
    ).await.expect("failed to connect to trojan server");

    let mut stream = stream;
    stream.write_all(b"hello world").await.unwrap();
    
    let mut buf = [0u8; 11];
    stream.read_exact(&mut buf).await.unwrap();
    
    assert_eq!(&buf, b"hello world");
}
